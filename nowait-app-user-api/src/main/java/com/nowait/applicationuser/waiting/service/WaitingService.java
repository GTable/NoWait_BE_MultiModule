...
public class WaitingService {
	...
	// 대기열 리팩토링 서비스 메서드
	@Transactional
	public RegisterWaitingResponse registerWaiting(CustomOAuth2User oAuth2User, Long storeId, RegisterWaitingRequest waitingRequest) {
		...
		waitingRedisRepository.indempotencyKeyExists(idempotentKey, ReservationStatus.WAITING.name());
	...
	@Transactional
	public void cancelWaiting(CustomOAuth2User oAuth2User, Long storeId) {
		// TODO 유저 및 주점 존재 검증은 공통으로 많이 쓰이니 AOP로 빼는게 좋을 듯
		Store store = storeRepository.findById(storeId)
			.orElseThrow(StoreNotFoundException::new);
		...
		String idempotentKey = generateIdempotentKey(storeId, user.getId());
		waitingRedisRepository.indempotencyKeyExists(idempotentKey, ReservationStatus.CANCELLED.name());

		// DB에 상태 값 저장
		// TODO: 웨이팅 취소 로직 구현 필요 (예: 상태 변경, 취소 시간 기록 등)
